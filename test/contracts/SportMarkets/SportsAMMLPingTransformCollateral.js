...
contract('SportsAMM', (accounts) => {
	...
	const ThalesContractForStaking = artifacts.require('contracts/Token/OpThales_L1.sol:OpThales');
	...
	beforeEach(async () => {
		...
		ThalesForStaking = await ThalesContractForStaking.new({ from: owner });
	...
	describe('Test SportsAMM LPing', () => {
		...
		it('Buy from SportsAMM, position 1, value: 100', async () => {
			...
			await Thales.mint(firstLiquidityProvider, toUnitSix('100'));
			await Thales.approve(SportAMMLiquidityPool.address, toUnitSix('100'), {
			...
			await SportAMMLiquidityPool.deposit(toUnitSix(100), { from: firstLiquidityProvider });
			...
			await Thales.mint(defaultLiquidityProvider, toUnitSix('1000'));
			await Thales.approve(SportAMMLiquidityPool.address, toUnitSix('1000'), {
			...
			await Thales.mint(secondLiquidityProvider, toUnitSix('1000'));
			await ThalesForStaking.transfer(secondLiquidityProvider, toUnit('1000'), {
				from: owner,
			});
			...
			await ThalesForStaking.approve(mockStakingThales.address, toUnit(1000), {
				from: secondLiquidityProvider,
			});
			...
			await expect(
				SportAMMLiquidityPool.deposit(toUnitSix(1000000), { from: secondLiquidityProvider })
			).to.be.revertedWith('Deposit amount exceeds AMM LP cap');

			await expect(
				SportAMMLiquidityPool.deposit(toUnitSix(101), { from: secondLiquidityProvider })
			).to.be.revertedWith('Not enough staked THALES');

			await expect(
				SportAMMLiquidityPool.deposit(toUnitSix(1), { from: secondLiquidityProvider })
